...
export function AdminOperationsPage() {
  ...
  const totals = useMemo(() => filteredPayments.reduce((value, item) => ({ gross: value.gross + item.monto_total_minor, platform: value.platform + item.comision_plataforma_minor, processing: value.processing + item.cargo_pasarela_minor, net: value.net + item.neto_negocio_minor }), { gross: 0, platform: 0, processing: 0, net: 0 }), [filteredPayments])
  ...
  return (
    <main className="page-container">
      ...
      {tab === 'payments' && <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(4, 1fr)' }, mb: 2.5 }}><Metric label="Volumen filtrado" value={formatMinorMoney(totals.gross, currency)} /><Metric label="Comisión Rapicancha" value={formatMinorMoney(totals.platform, currency)} /><Metric label="Cargo de pasarela" value={formatMinorMoney(totals.processing, currency)} /><Metric label="Neto clubes" value={formatMinorMoney(totals.net, currency)} /></Box>}
