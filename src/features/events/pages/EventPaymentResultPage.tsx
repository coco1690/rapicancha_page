...
function Summary({ registration }: { registration: NonNullable<ReturnType<typeof useEventPaymentResultStore.getState>['registration']> }) {
  ...
  return <Box sx={{ borderBlock: 1, borderColor: 'divider', width: '100%' }}><Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>{items.map((item) => <Stack direction="row" key={item.label} spacing={1} sx={{ alignItems: 'center', minWidth: 0, p: 1.5 }}><Avatar sx={{ bgcolor: 'action.hover', color: 'primary.main', height: 34, width: 34 }}>{item.icon}</Avatar><Box sx={{ minWidth: 0, textAlign: 'left' }}><Typography color="text.secondary" sx={{ fontSize: 10.5, fontWeight: 800 }}>{item.label}</Typography><Typography sx={{ fontSize: 13, fontWeight: 900, overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.value}</Typography></Box></Stack>)}</Box>{registration.registrations?.length ? <Stack spacing={0.75} sx={{ borderTop: 1, borderColor: 'divider', p: 1.5 }}>{registration.registrations.map((item) => <Stack direction="row" key={item.number} sx={{ justifyContent: 'space-between', textAlign: 'left' }}><Box><Typography variant="body2" sx={{ fontWeight: 850 }}>{item.participantName}</Typography><Typography color="text.secondary" variant="caption">{item.number}</Typography></Box><Typography color="primary.main" sx={{ fontWeight: 950 }}>{item.bib ? `Dorsal ${item.bib}` : 'Confirmado'}</Typography></Stack>)}</Stack> : registration.bib && <Typography sx={{ bgcolor: 'secondary.light', p: 1.5, fontWeight: 950 }}>Dorsal asignado: {registration.bib}</Typography>}</Box>
}
